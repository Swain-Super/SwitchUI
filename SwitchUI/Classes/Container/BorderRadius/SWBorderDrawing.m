#import "SWBorderDrawing.h"

NS_ASSUME_NONNULL_BEGIN

typedef void (^KTGraphicsImageDrawingActions)(CGContextRef _Nullable context);
...
static UIImage *_Nullable getSolidBorderImage(SWCornerRadii cornerRadii, CGSize viewSize, UIEdgeInsets borderInsets, SWBorderColors borderColors, CGColorRef _Nullable backgroundColor, BOOL drawToEdge);

static UIImage *_Nullable getDashedOrDottedBorderImage(SWBorderStyle borderStyle, SWCornerRadii cornerRadii, CGSize viewSize, UIEdgeInsets borderInsets, SWBorderColors borderColors, CGColorRef _Nullable backgroundColor, BOOL drawToEdge);

NS_INLINE BOOL cornerRadiiAreAboveThreshold(SWCornerRadii cornerRadii);

static CGContextRef _Nullable uiGraphicsBeginImageContext(CGSize size, BOOL opaque);
static UIImage *_Nullable uiGraphicsImageRendererToDrawing(KTGraphicsImageDrawingActions _Nullable uiactions, CGSize size, BOOL opaque);

static UIEdgeInsets roundInsetsToPixel(UIEdgeInsets edgeInsets);

static CGPathRef pathCreateOuterOutline(BOOL drawToEdge, CGRect rect, SWCornerRadii cornerRadii);
...
/// MARK: - Util
static CGFloat SWScreenScale(void);

static CGFloat SWRoundPixelValue(CGFloat value);
...
CGFloat KTRoundPixelValue(CGFloat value) {
    CGFloat scale = KTScreenScale();

    return round(value * scale) / scale;
}

UIEdgeInsets roundInsetsToPixel(UIEdgeInsets edgeInsets) {
    edgeInsets.top = KTRoundPixelValue(edgeInsets.top);
    edgeInsets.bottom = KTRoundPixelValue(edgeInsets.bottom);
    edgeInsets.left = KTRoundPixelValue(edgeInsets.left);
    edgeInsets.right = KTRoundPixelValue(edgeInsets.right);
...
CGPathRef pathCreateOuterOutline(BOOL drawToEdge, CGRect rect, SWCornerRadii cornerRadii) {
    if (drawToEdge) {
        return CGPathCreateWithRect(rect, nil);
    }

    // 圆角外轮廓
    return SWPathCreateWithRoundedRect(rect, SWGetCornerInsets(cornerRadii, UIEdgeInsetsZero), nil);
}

static const CGFloat SW_VIEW_BORDER_THRESHOLD = 0.001;

BOOL cornerRadiiAreAboveThreshold(SWCornerRadii cornerRadii) {
    return cornerRadii.topLeft > SW_VIEW_BORDER_THRESHOLD || cornerRadii.topRight > SW_VIEW_BORDER_THRESHOLD || cornerRadii.bottomLeft > SW_VIEW_BORDER_THRESHOLD || cornerRadii.bottomRight > SW_VIEW_BORDER_THRESHOLD;
}

BOOL SWBorderInsetsAreEqual(UIEdgeInsets borderInsets) {
    return ABS(borderInsets.left - borderInsets.right) < SW_VIEW_BORDER_THRESHOLD && ABS(borderInsets.left - borderInsets.bottom) < SW_VIEW_BORDER_THRESHOLD && ABS(borderInsets.left - borderInsets.top) < SW_VIEW_BORDER_THRESHOLD;
}

BOOL SWCornerRadiiAreEqual(SWCornerRadii cornerRadii) {
    return ABS(cornerRadii.topLeft - cornerRadii.topRight) < SW_VIEW_BORDER_THRESHOLD && ABS(cornerRadii.topLeft - cornerRadii.bottomLeft) < SW_VIEW_BORDER_THRESHOLD && ABS(cornerRadii.topLeft - cornerRadii.bottomRight) < SW_VIEW_BORDER_THRESHOLD;
}

BOOL SWBorderColorsAreEqual(SWBorderColors borderColors) {
...
SWCornerInsets SWGetCornerInsets(SWCornerRadii cornerRadii, UIEdgeInsets edgeInsets) {
    return (SWCornerInsets) {
            {
...
UIImage *getSolidBorderImage(SWCornerRadii cornerRadii, CGSize viewSize, UIEdgeInsets borderInsets, SWBorderColors borderColors, CGColorRef backgroundColor, BOOL drawToEdge) {
    const BOOL hasCornerRadii = cornerRadiiAreAboveThreshold(cornerRadii);
    const SWCornerInsets cornerInsets = SWGetCornerInsets(cornerRadii, borderInsets);
    ...
    KTGraphicsImageDrawingActions uiactions = ^(CGContextRef ctx) {
        ...
        CGPathRef insetPath = SWPathCreateWithRoundedRect(UIEdgeInsetsInsetRect(rect, borderInsets), cornerInsets, nil);

        CGContextAddPath(ctx, insetPath);
        CGContextEOClip(ctx);

        BOOL hasEqualColors = SWBorderColorsAreEqual(borderColors);
...
UIImage *getDashedOrDottedBorderImage(SWBorderStyle borderStyle, SWCornerRadii cornerRadii, CGSize viewSize, UIEdgeInsets borderInsets, SWBorderColors borderColors, CGColorRef backgroundColor, BOOL drawToEdge) {
    NSCParameterAssert(borderStyle == SWBorderStyleDashed || borderStyle == SWBorderStyleDotted);

    if (!SWBorderColorsAreEqual(borderColors) || !SWBorderInsetsAreEqual(borderInsets)) {
        // TODO(ChasonTang): SWLogWarn(@"Unsupported dashed / dotted border style");
        return nil;
    }
    ...
    KTGraphicsImageDrawingActions uiactions = ^(CGContextRef ctx) {
        ...
        CGPathRef path = SWPathCreateWithRoundedRect(pathRect, SWGetCornerInsets(cornerRadii, UIEdgeInsetsZero), nil);

        CGFloat dashLengths[2];
        dashLengths[0] = dashLengths[1] = (borderStyle == SWBorderStyleDashed ? 3 : 1) * lineWidth;
...
UIImage *uiGraphicsImageRendererToDrawing(KTGraphicsImageDrawingActions uiactions, CGSize size, BOOL opaque) {
    UIImage *image = nil;
...
CGPathRef SWPathCreateWithRoundedRect(CGRect bounds, SWCornerInsets cornerInsets, const CGAffineTransform *transform) {
...
UIImage *SWGetBorderImage(SWBorderStyle borderStyle, CGSize viewSize, SWCornerRadii cornerRadii, UIEdgeInsets borderInsets, SWBorderColors borderColors, CGColorRef backgroundColor, BOOL drawToEdge) {
    switch (borderStyle) {
        case SWBorderStyleSolid:
            return getSolidBorderImage(cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
        case SWBorderStyleDashed:
        case SWBorderStyleDotted:
            return getDashedOrDottedBorderImage(
                    borderStyle, cornerRadii, viewSize, borderInsets, borderColors, backgroundColor, drawToEdge);
        case SWBorderStyleNone:
            break;
    }
